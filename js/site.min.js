...
(function() {
    ...
    if (window.devicePixelRatio >= 2) {
        var images = document.getElementsByClassName('retina-image');
        if (images.length > 0) {
...
(function($) {
    'use strict';
    // Iron Sticky Header
    $.fn.iron_sticky_header = function(options) {
        // Options
        var settings = $.extend({
            scroll_amount: 254,
            class_name: 'o-sticky-header_js'
        }, options);
        // On Page Init - Adds/Removes Class name
        function iron_scrolling(e, scroll_amount, class_name) {
            if ($(e).scrollTop() >= (scroll_amount)) {
                $('body').addClass(class_name);
                $('#site-header-wrapper .main-menu-wrapper .sub-menu').css({
                    display: 'none'
                });
            } else {
                $('body').removeClass(class_name);
                $('.site-sticky-header .sub-menu').css({
                    display: 'none'
                });
            }
        }
        iron_scrolling(this, settings.scroll_amount, settings.class_name);
        // On Scroll - Adds/Removes Class name
...
window.onload = function() {
    jQuery(document).ready(function($) {
        'use strict';
        var mobile_menu = $('#site-header-wrapper .main-menu').clone();
        // Sticky Header
        var site_header = $('#site-header-wrapper > .container').clone();
        $('body').append('<div class="site-sticky-header"></div>');
        site_header.appendTo('.site-sticky-header');
        $('.site-sticky-header').find('#menu-main-menu').attr('id', 'menu-main-menu2');
        $('.site-sticky-header').find('li').attr('id', null);
        $(window).iron_sticky_header();
        // Main Menu - Superfish
        $('.main-menu-wrapper > .main-menu').superfish({
            speed: 'fast',
            delay: 400
        });
        // Mobile Menu Popup
        $('.site-mobile-menu-hamburger').on('click', function() {
            $('.site-mobile-menu-popup').addClass('o-active_js');
        });
        $('body').append('<div class="site-mobile-menu-popup"><div class="e_menu"><div class="e_menu_close"><i class="fa fa-times" aria-hidden="true"></i></div></div></div>');
        mobile_menu.appendTo('.site-mobile-menu-popup .e_menu');
        $('.site-mobile-menu-popup').find('#menu-main-menu').attr('id', 'menu-main-menu3');
        $('.site-mobile-menu-popup').find('li').attr('id', null);
        $('.site-mobile-menu-popup .menu-item-has-children > a').append('<span class="ss-toggle"></span>');
        $('.ss-toggle').on('click', function(e) {
            $('.site-mobile-menu-popup .ss-toggle').not($(this)).removeClass('o-active_js');
            $('.site-mobile-menu-popup .main-menu .sub-menu').not($(this).parent().next()).slideUp();
            $(this).toggleClass('o-active_js');
            $(this).parent().next().slideToggle();
            e.preventDefault();
        });
        $('.site-mobile-menu-popup .e_menu_close').on('click', function() {
            $('.site-mobile-menu-popup').removeClass('o-active_js');
        });
        // Site Search
        $('.site-search-icon').on('click', function() {
            console.log('search clikced')
            $('.site-search').addClass('o-active_js');
        });
        $('.site-search .e_close').on('click', function() {
            $('.site-search').removeClass('o-active_js');
        });
        // Image Horizontal Scroller
        $('.ost_image_hor_scroller .e_slider').bxSlider({
            slideMargin: 31,
            minSlides: 1,
            maxSlides: 20,
            moveSlides: 1,
            slideWidth: 150,
            prevText: '<i class="fa fa-angle-left" aria-hidden="true"></i><span>Previous Slide</span>',
            nextText: '<i class="fa fa-angle-right" aria-hidden="true"></i><span>Next Slide</span>',
            onSliderLoad: function() {
                $(this).parents('.ost_image_hor_scroller').addClass('o-loaded_js');
            }
        });
        /*
        $('.ost_image_hor_scroller').mCustomScrollbar({
            axis:"x",
            theme:"light-3",
        ...
        */
        // Posts Slider
        $('.ost_posts_slider .e_posts').bxSlider({
            infiniteLoop: false,
            slideMargin: 16,
            minSlides: 1,
            maxSlides: 3,
            ...
            nextText: '<i class="fa fa-angle-right" aria-hidden="true"></i><span>Next Slide</span>',
            onSliderLoad: function() {
                $(this).parents('.ost_posts_slider').addClass('o-loaded_js');
            }
        });
        // Anchor Scroll
        $('a').on('click', function(e) {
            if ($(this).attr('href').indexOf('#') === 0 && $($(this).attr('href')).length) {
                ...
                $('html, body').stop().animate({
                    scrollTop: $($(this).attr('href')).offset().top - ($('.site-sticky-header').outerHeight() + $('#wpadminbar').outerHeight())
                }, 600);
                e.preventDefault();
            }
        });
        // Page Load - if current page url has #hash; scroll to element
        var current_page_hash = window.location.hash;
        if (current_page_hash) {
            if ($(current_page_hash).length) {
                setTimeout(function() {
                    $('html, body').stop().animate({
                        scrollTop: $(current_page_hash).offset().top - ($('.site-sticky-header').outerHeight() + $('#wpadminbar').outerHeight())
                    }, 600);
                }, 100);
            }
        }
        // Site Load More
        function button_status(e, f, t) {
            e.css('pointer-events', f);
            if ('none' === f) {
                e.html(t);
            } else {
                e.html(t);
        ...
        function site_load_more() {
            $('.e_load_more').on('click', function(e) {
                var elem = $(this);
                var container = $(this).parent().prev();
                var args = [];
                // Query Args
                elem.data('paged', elem.data('paged') + 1);
                args = elem.data();
                button_status(elem, 'none', '<i class="fa fa-spinner fa-pulse fa-lg fa-fw"></i>');
                // Ajax
                $.ajax({
                    url: ironajax.ajax_url,
                    type: 'post',
                    data: {
                        action: 'site_load_more',
                        args: args
                    },
                    success: function(response) {
                        console.log(response);
                        var posts = $.parseHTML(response.data.output);
                        if (response.data.output) {
                            ...
                            $(posts).imagesLoaded(function() {
                                ...
                                setTimeout(function() {
                                    container.find('.e_item > div').matchHeight();
                                    button_status(elem, 'auto', 'Load More');
                                }, 10);
                            });
                        } else {
                            button_status(elem, 'none', 'Done');
                        }
                    }
                });
                e.preventDefault();
